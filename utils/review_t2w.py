        ...
        for filename in tqdm(valid_images, desc="Moving valid images"):
            ...
            if os.path.isfile(source_path):
                ...
                if os.path.isfile(destination_path):
                    ...
                    print(f"File {source_path} does not exist in the source directory and was not moved previously.")
        ...
        for (patient_id, scan_id), file_paths in patient_scan_files.items():
            if len(file_paths) > 1:  # only consider scans with more than one version
                ...
                for file_path in file_paths:
                    ...
                    axial_res = img.header['pixdim'][1:3].prod()  # Axial resolution is the product of the first two pixel dimensions
                    if axial_res > max_res:
                        max_res = axial_res
                        max_res_file = file_path
                print(f"Highest resolution version for patient {patient_id}, scan {scan_id} is {max_res_file}")
    ...
    def main(self):
        for suffix, csv_file in self.csv_files.items():
            ...
            if detecting:
                ...
                output_detection_folders = [review_cfg.OUTPUT_DETECTTION_60, review_cfg.OUTPUT_DETECTTION_29]
