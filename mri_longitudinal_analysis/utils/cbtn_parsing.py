...
def read_csv(path_cbtn) -> (list, list):
    ...
    assert df_cbtn["Specimen Collection Origin"] == (
        "Initial CNS Tumor Surgery" or "Not Applicable"
    )
    assert len(df_cbtn) == 536  # this is fix from previous filtering in the csv file
...
def extract_number(s):
    """
    Function to extract the number before 'd' in the string.s
    """
...
def get_session_label(path_mr_cbtn, ages):
    list_ages = []
    nmidonama = []
...
def get_closest_ages(ages, list_ages):
    choosen_ages = []
...
def get_t2_sequences(path_mr_cbtn, meta_data, choosen_ages):
    """
    ...
    session_label = []
    for i, item in enumerate(path_mr_cbtn):
        for j, row in enumerate(meta_data["subject_label"]):
            if choosen_ages[i] != []:
                condition = (meta_data["subject_label"] == item[27:]) & (
                    meta_data["age_at_imaging_in_days"] == choosen_ages[i]
                )
                filtered_df = meta_data[condition]
                ...
                if filtered_df.shape[0] > 0:
                    ...
                    if containing_1:
                        ...
                        if containing_2:
                            ...
                            containing_3 = [
                                seq for seq in label_n if "T2" in seq and "AXIAL" in seq
                            ]
                            if containing_3:
                                ...
                                containing_4 = [
                                    seq for seq in label_n if "T2" in seq and "Ax" in seq
                                ]
                                if containing_4:
...
def copy_images(path_mr_cbtn, session_label, seq, new_path_cbtn):
...
def main():
    ...
    ages, path_mr_cbtn = read_csv(path_cbtn)
