...
class MixedEffectsModel:
    ...
    def __init__(self, mg_data):
        ...
        self.models = {}
        self.formula = "Patient_Classification_Binary ~ Age + Volume + Volume_Change_Rate"
    ...
    def preprocess_data_for_training(self, data, outcome_var):
        ...
        """
        train_data, test_data = train_test_split(data, test_size=0.1, random_state=42, stratify=data[outcome_var])
        train_data_index = train_data.index

        # Oversampling using Random Oversampling (ROS)
        ros = RandomOverSampler(random_state=42)
        X_train_resampled, y_train_resampled = ros.fit_resample(train_data[['Age', 'Volume', 'Volume_Change_Rate']], train_data[outcome_var])
        train_data_oversampled = pd.DataFrame(X_train_resampled, columns=['Age', 'Volume', 'Volume_Change_Rate'])
        ...
        train_data_oversampled['Patient_ID'] = train_data.loc[train_data_oversampled.index, 'Patient_ID'].values        
        
        # Undersampling using Random Undersampling (RUS)
        rus = RandomUnderSampler(random_state=42)
        X_train_resampled, y_train_resampled = rus.fit_resample(train_data[['Age', 'Volume', 'Volume_Change_Rate']], train_data[outcome_var])
        train_data_undersampled = pd.DataFrame(X_train_resampled, columns=['Age', 'Volume', 'Volume_Change_Rate'])
        ...
        train_data_undersampled['Patient_ID'] = train_data.loc[train_data_undersampled.index, 'Patient_ID'].values
   
        smote = SMOTE(random_state=42)
        X_train_resampled, y_train_resampled = smote.fit_resample(train_data[['Age', 'Volume', 'Volume_Change_Rate']], train_data[outcome_var])
        train_data_smote = pd.DataFrame(X_train_resampled, columns=['Age', 'Volume', 'Volume_Change_Rate'])
        train_data_smote['Patient_Classification_Binary'] = y_train_resampled
        train_data_smote['Patient_ID'] = np.random.choice(train_data['Patient_ID'].unique(), size=len(train_data_smote))

        return train_data, test_data, train_data_oversampled, train_data_undersampled, train_data_smote
    
    #################
    # MODEL FITTING #
    #################
    def run_mixed_effects_model(self, data, grouping_var, longi_vars, individual_model_dir, mixed_model_dir):
        """
        ...
        for var in longi_vars:
            ...
            try:
                ...
                ind_summary_file = os.path.join(individual_model_dir, f'{var}_summary.txt')
        ...
        all_models = self.interaction_models(data, grouping_var, longi_vars, interaction_terms)
        for name, model in all_models.items():
            model_summary_file = os.path.join(mixed_model_dir, f'{str(name)}_summary.txt')
    ...
    def compare_models(self, models):
        ...
        for name, model in models.items():
            ...
            if np.isnan(model.params).any() or np.isinf(model.params).any():                
                print(f"Skipping model '{name}' due to NaN or infinite parameter estimates.")
                continue
    ...
    def train_models(self, train_data, train_data_oversampled, train_data_undersampled, train_data_smote, outcome_var, trained_model):
        """
        Train and fit models on the data.
        """        
        # Vanilla Model
        class_weights = class_weight.compute_class_weight('balanced', classes=np.unique(train_data[outcome_var]), y=train_data[outcome_var])
        class_weights_dict = dict(enumerate(class_weights))
        train_data['class_weights'] = train_data[outcome_var].map(class_weights_dict)
        model = smf.mixedlm(formula=self.formula, data=train_data, groups=train_data["Patient_ID"], re_formula=self.re_formula)
        #pylint: disable=unexpected-keyword-arg
        ...
        model_oversampling = smf.mixedlm(formula=self.formula, data=train_data_oversampled, groups=train_data_oversampled['Patient_ID'], re_formula=self.re_formula)
        self.models['Oversampling'] = model_oversampling.fit(reml=False)

        # Model with undersampling
        model_undersampling = smf.mixedlm(formula=self.formula, data=train_data_undersampled, groups=train_data_undersampled['Patient_ID'], re_formula=self.re_formula)
        self.models['Undersampling'] = model_undersampling.fit(reml=False)
        
        # Model with SMOTE
        model_smote = smf.mixedlm(formula=self.formula, data=train_data_smote, groups=train_data_smote['Patient_ID'], re_formula=self.re_formula)
        self.models['SMOTE'] = model_smote.fit(reml=False)

        # Random Forest Classifier
        rf_model = RandomForestClassifier(n_estimators=300, class_weight='balanced', random_state=42, min_samples_leaf=2, min_samples_split=5, max_depth=None)
        rf_model.fit(train_data[['Age', 'Volume', 'Volume_Change_Rate']], train_data[outcome_var])
        self.models['Random Forest'] = rf_model

        rf_model_smote = RandomForestClassifier(n_estimators=300, class_weight='balanced', random_state=42, min_samples_leaf=2, min_samples_split=5, max_depth=None)
        rf_model_smote.fit(train_data_smote[['Age', 'Volume', 'Volume_Change_Rate']], train_data_smote[outcome_var])
    ...
    def evaluate_models(self, test_data, outcome_var, debug=False):
        ...
        for model_name, model in self.models.items():
            if model_name == 'Random Forest' or model_name == 'RF SMOTE':
                test_data['pred_prob'] = model.predict_proba(test_data[['Age', 'Volume', 'Volume_Change_Rate']])[:, 1]
            else:
                test_data['pred_prob'] = model.predict(test_data)
            best_threshold = None
            best_score = 0
            for threshold in thresholds:
                test_data[f'pred_class_{threshold}'] = np.where(test_data['pred_prob'] >= threshold, 1, 0)
                if debug:
                    print(f"Model: {model_name}")
                    print(f"Threshold: {threshold}")
                    print(classification_report(test_data[outcome_var], test_data[f'pred_class_{threshold}']))
                    print()
                score = f1_score(test_data[outcome_var], test_data[f'pred_class_{threshold}'])
                score_name = 'F1 Score'
                if score > best_score:
                    best_score = score
        ...
        if best_model == 'Random Forest' or best_model == 'RF SMOTE': 
            test_data['pred_prob'] = self.models[best_model].predict_proba(test_data[['Age', 'Volume', 'Volume_Change_Rate']])[:, 1]
        else:
            test_data['pred_prob'] = self.models[best_model].predict(test_data[['Age', 'Volume', 'Volume_Change_Rate']])
        test_data['pred_class'] = np.where(test_data['pred_prob'] >= best_model_threshold, 1,0)
    ...
    def grid_search(self, train_data, outcome_var):
        ...
        # Perform grid search
        grid_search = GridSearchCV(estimator=rf_model, param_grid=self.param_grid, scoring='f1', cv=5, n_jobs=-1)
        grid_search.fit(train_data[['Age', 'Volume', 'Volume_Change_Rate']], train_data[outcome_var])
    ...
    def visualize_mixed_model_effects(self, data, model_result, variable, output_dir):
        ...
        ax2.errorbar(x=0, y=coef, xerr=[[coef - ci_lower], [ci_upper - coef]], fmt='o', color='blue', label='Fixed Effect')
        ...
        ax3.scatter(random_effects_vals, np.arange(len(random_effects_vals)), color='green', alpha=0.5)
    ...
    def main(self, output_dir):
        ...
        for var in longitudinal_vars:
            if var == "Volume_Change":
                continue
            self.visualize_mixed_model_effects(data, combined_model, var, individual_model_dir)
        ...
        train_data, test_data, train_data_oversampled, train_data_undersampled, train_data_smote = self.preprocess_data_for_training(data, outcome_var)
        self.train_models(train_data, train_data_oversampled, train_data_undersampled, train_data_smote, outcome_var, best_mixed_effects_model)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "/home/jc053/GIT/mri_longitudinal_analysis/data/output/correlation_plots_joint"
