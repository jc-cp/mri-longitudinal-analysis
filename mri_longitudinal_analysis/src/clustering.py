...
class ClusterAnalysis:
    def __init__(self, dataframe, patient_id_column, output_path, metrics_output_path):
        ...
        self.data_to_cluster = self.data.drop(
            columns=[self.patient_id_column]
    ...
    def apply_tsne(self, n_components=2):
        self.tsne_embedding = TSNE(n_components=n_components).fit_transform(
    ...
    def plot_clusters(self, suffix):
        ...
        plt.colorbar(
            boundaries=np.arange(
                min(self.cluster_labels) - 0.5, max(self.cluster_labels) + 1.5
            )
    ...
    def plot_heatmap(self, suffix):
        ...
        # Create a heatmap
        ax = sns.heatmap(sorted_data, cmap="coolwarm", annot=True, linewidths=0.5)
...
if __name__ == "__main__":
    ...
    metrics_output_path = (
        "/home/jc053/GIT/mri-longitudinal-analysis/data/output/metrics.txt"
    )

    cluster_analysis = ClusterAnalysis(
        all_data, "patient_id", output_path, metrics_output_path
    )
    ...
    for method, (func, suffix) in tqdm(
